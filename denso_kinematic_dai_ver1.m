function [p_c,R_c,p_troc,R_troc,J_grip,J_denso,J_rcm_p]=denso_kinematic_dai_ver1(joint_v,troc_para)
...
t_theta2=troc_para(2); % trocar圆弧圆心角
t_r=troc_para(3); % 圆弧半径

d1=475;%dai：paper里是473
a1=180; d2=150+96;%dai：paper里是0
d3=d2;a2=385; a3=100; d4=445;d6=90;L=40;
x_axis=[1;0;0]; y_axis=[0;1;0]; z_axis=[0;0;1];
%%%%%%%%%%
R_0_1=rotz(pi/2+q1); %dai：与paper也画的不一样，应该没有pi/2
p_0_o1=[0;0;d1];
...
R_0_t2=R_0_6*R_6_t2;
p_6_o6t2=180*z_axis+[0;t_r-cos(t_theta2)*t_r;sin(t_theta2)*t_r];
p_0_o6t2=R_0_6*p_6_o6t2;

R_t2_7=rotz(-delta)*roty(theta)*rotz(delta);
R_6_7=R_6_t2*R_t2_7;
R_0_7=R_0_t2*R_t2_7;
...
if theta>0
    ...
    p_0_o7=p_0_o6+p_0_o6t2+R_0_t2*p_t2_c;
else
    p_0_o7=p_0_o6+p_0_o6t2+R_0_t2*[0;0;L]; %theta永远不会小于0
end
...
R_troc=R_0_6*R_6_t1; 
p_troc=p_0_o6+R_0_6*p_6_o6t1;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Jacobian
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if theta>0
% Jcv=R_0_6*([L*sin_delta*(theta*sin_theta+cos_theta-1)/theta^2,L*sin_delta*(cos_theta-1)/theta;...%这里第一项应该是cos_delta而不是sin_delta!!!!
...
p_6_o6g=p_6_o6t2+L/theta*[cos_delta*(1-cos_theta);sin_delta*(cos_theta-1);sin_theta]+R_6_7*[0;0;15];
else 
%     Jcv=R_0_t2*[(L+15)/2*cos_delta,0;-(L+15)/2*sin_delta,0;0,0];% 这里不应该直接在L上加gripper长度15
...
cross_z=[0,-1,0;1,0,0;0,0,0]; % z_zaxis的反对称矩阵
