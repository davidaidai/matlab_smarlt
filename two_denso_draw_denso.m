function [cylinderhandle, cubehandle, cylinder_handles, linehandles, gripperhandle, coo_handle,trocarhandle] = two_denso_draw_denso(figure_handle,mov_p_o0,joint_v,troc_para,draw_flag ,denso_cylinderhandle,denso_cubehandle,denso_chandle,denso_lhandle,denso_ghandle,coo_handle,trocarhandle)
...
switch size(troc_para,2)
    case 3
        s_troc=troc_para(2); % trocar弧长
        troc_r=troc_para(3); % 圆弧半径
        R_6_t2=rotx(-s_troc/troc_r); % troc末端在6坐标系中的指向
        R_0_t2=R_0_6*R_6_t2;
        draw_t_theta=linspace(0,s_troc/troc_r,npts);
        p_6_t2=180*z_axis*ones(1,npts)+[zeros(1,npts);troc_r-cos(draw_t_theta)*troc_r;sin(draw_t_theta)*troc_r];
        p_0_t2=p_0_o6*ones(1,npts)+R_0_6*p_6_t2;
    case 4
        s_troc=troc_para(2); % trocar弧长
        troc_r_fir=troc_para(3); % 第一段圆弧半径
        troc_r_sec=troc_para(4); % 第二段圆弧半径
        R_6_t2=rotx(-s_troc/2/troc_r_fir)*rotx(-s_troc/2/troc_r_sec); % troc末端在6坐标系中的指向
        R_0_t2=R_0_6*R_6_t2;
        draw_t_theta_fir=linspace(0,s_troc/2/troc_r_fir,npts/2);
        draw_t_theta_sec=linspace(0,s_troc/2/troc_r_sec,npts/2);
        p_6_t2_fir=180*z_axis*ones(1,npts/2)+[zeros(1,npts/2);troc_r_fir-cos(draw_t_theta_fir)*troc_r_fir;sin(draw_t_theta_fir)*troc_r_fir];
        p_6_t2_sec=p_6_t2_fir(:,npts/2)*ones(1,npts/2)+rotx(-s_troc/2/troc_r_fir)*[zeros(1,npts/2);troc_r_sec-cos(draw_t_theta_sec)*troc_r_sec;sin(draw_t_theta_sec)*troc_r_sec];
...
if strcmp(draw_flag,'draw')
    ...
    %     cylinderhandle13=draw_cylinder(figure_handle,p_0_o6b,rad_link6b*R_0_6*x_axis,400*R_0_6*z_axis,'r',transprancy,'draw');% 直的圆柱形trocar
    hold on
    trocarhandle=plot3(p_0_t2(1,:),p_0_t2(2,:),p_0_t2(3,:),'g','LineWidth',5);
...
if strcmp(draw_flag,'update')
    ...
    set(trocarhandle,'XData',p_0_t2(1,:),'YData',p_0_t2(2,:),'ZData',p_0_t2(3,:));
