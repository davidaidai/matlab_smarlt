function [cylinderhandle, cubehandle, cylinder_handles, linehandles, gripperhandle, coo_handle,trocarhandle] = draw_denso(figure_handle, joint_v,troc_para,draw_flag ,denso_cylinderhandle,denso_cubehandle,denso_chandle,denso_lhandle,denso_ghandle,coo_handle,trocarhandle)
...
t_theta1=troc_para(1); % RCMÔ²»¡Ô²ĞÄ½Ç
t_theta2=troc_para(2); % trocarÔ²»¡Ô²ĞÄ½Ç
t_r=troc_para(3); % Ô²»¡°ë¾¶
...
% trocar
npts=100;
R_6_t2=rotx(-t_theta2);
R_0_t2=R_0_6*R_6_t2;
draw_t_theta=linspace(0,t_theta2,npts);
p_6_t2=180*z_axis*ones(1,npts)+[zeros(1,npts);t_r-cos(draw_t_theta)*t_r;sin(draw_t_theta)*t_r]; %ÓĞÒ»¸ö¹Ì¶¨180mmÑØz6×ø±êÖáµÄÆ«ÒÆ£¬ÕâÊÇarmµÄÇı¶¯¶Ë
...
if strcmp(draw_flag,'draw')
    ...
    %link4
    cylinderhandle8=draw_cylinder(figure_handle,p_0_o4,rad_link4*R_0_4*x_axis,-330.6*R_0_4*z_axis,'c',transprancy,'draw');
...
cylinderhandle8=draw_cylinder(figure_handle,p_0_o4,rad_link4*R_0_4*x_axis,-330.6*R_0_4*z_axis,'c',transprancy,'update',denso_cylinderhandle(8,:));
